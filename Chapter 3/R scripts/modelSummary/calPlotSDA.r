...
df1 <- df1 %>% filter(TREAT_GRP4 == "SDA")

data2 <- prog2$mice$result_grp
df2 <- complete(data2, action = "long")
df2 <- df2 %>% filter(TREAT_GRP4 == "SDA")

# STEP 2: Calculate linear predictor across imputed datasets (ignoring clustering term) and attach to stacked dataset
...
saveRDS(df_joined1, file = "data/calM1PredictionsSDA.rds")
saveRDS(df_joined2, file = "data/calM2PredictionsSDA.rds")
...
x <- 0.8
ggsave(
  filename = "graphs/SDA/calPlotSDA.pdf", plot = cal, width = 12.15 * x, height = 12.15 * x / 2, dpi = 600
)
