...
prog <- readRDS("results-share/originalModel_WITHOUT_PD_JAN2026_updated.rds")
sum <- readRDS("results-share/202605_WITHOUT_PD_BOOT.rds")
...
df <- bind_rows(df2, df3, df4, df5, df6)
df <- df %>%
  select(INTERCEPT, DM_AGEs, ZZ_AGEs2, LB_BL_HGB_GRP3, VL_DURATIONs, TREAT_GRP4SDA, TREAT_GRP4OTHER, AGE, FEVER_DUR, VAR)
...
# align plots
p_list <- list(fever_dur, treat, anaemia)
...
x <- 1.3
ggsave(filename = "figures/multiAssocM2.pdf", plot = final_plot, width = 20 * x, height = 6 * x, dpi = 1200, units = "cm")
