...
miceWrapper <- function(data, predictorMatrix, method, m, seed = NA, maxit, post, file = "default", ...) {
  # initialise
  warn_ind <- 1
  warnings <- data.frame(matrix(ncol = 5, nrow = 0))
  colnames(warnings) <- c("Iteration", "Imputation", "Variable", "Time", "Message")
  current_state <<- list(iter = integer(1), imp = integer(1), variable = character(1)) # need this accessible within mice namespace environment!
  result <- NULL
  time_start <- Sys.time()

  # catch and log warnings and errors
  tryCatch(
    {
      withCallingHandlers(
        {
    ...
    error = function(e) {
      ...
      time <- list(time_start = time_start, time_end = Sys.time())
      list(result = NULL, warnings = warnings, error = e$message, time = time) # Return error message
    },
    finally = {
