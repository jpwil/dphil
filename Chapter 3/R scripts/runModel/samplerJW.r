...
sampler <- function(
    data, m, ignore, where, imp, blocks, method, visitSequence,
  ...
  maxit <- to - from + 1
  r <- !is.na(data)
  chainMean <- chainVar <- mice:::initialize.chain(
    blocks, maxit,
    m
  )
  if (maxit < 1) {
    ...
    for (k in from:to) {
      ...
      for (i in seq_len(m)) {
        ...
        for (h in visitSequence) {
          ...
          newstate <- list(
            it = k, im = i, dep = h, meth = theMethod,
            log = oldstate$log
          )
  ...
  list(
    iteration = maxit, imp = imp, chainMean = chainMean,
    chainVar = chainVar
  )
}
