...
# bs_num is the number of bootstraps
# cluster (boolean true/false) determines whether to bootstrap at the cluster level or not

bootstrapData <- function(df, bs_num, cluster) {
  # define helper functions
  mi_bootstrap_ind <- function(df, bs_num) {
    df_boot_ind <- vector("list", bs_num)
    for (i in 1:bs_num) {
      df_boot_ind[[i]] <- df[sample(nrow(df), replace = TRUE), ] # resample with replacement
    }
    df_boot_ind # return value
  }
  ...
  mi_bootstrap_cluster <- function(df, bs_num) {
    ...
    for (j in 1:bs_num) {
      resample <- sample(seq_along(unique(df$ORIG_STUDYID)), replace = TRUE) # sample the study IDs with replacements
