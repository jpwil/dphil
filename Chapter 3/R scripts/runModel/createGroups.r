...
createGroups <- function(prog) {
  if (is.null(prog$mice$result)) {
    cat("\nMICE FAILED\n")
    return(NULL)
  }

  long_data <- mice::complete(prog$mice$result, action = "long", include = TRUE)
  data_scale <- prog$data_scale


  long_data <- long_data %>% mutate(STUDYID = factor(STUDYID))
  long_data <- long_data %>%
    dplyr::mutate(
      DM_AGE = convertToOriginal(DM_AGEs, "DM_AGE", data_scale = data_scale),
      LB_BL_HGB = convertToOriginal(LB_BL_HGBs, "LB_BL_HGB", data_scale = data_scale),
      ZZ_BMI = convertToOriginal(ZZ_BMIs, "VS_BMI", data_scale = data_scale)
    )
  ...
  # for BMI-z scores, defined for age < 19:
  ...
  long_data <- long_data %>%
    mutate(
      ZZ_BMI_Z_GRP = case_when(
        DM_AGE < 19 & ZZ_BMI_Z > -2 & !is.na(ZZ_BMI_Z) ~ "Mild_norm",
        DM_AGE < 19 & ZZ_BMI_Z <= -2 & ZZ_BMI_Z > -3 & !is.na(ZZ_BMI_Z) ~ "Moderate",
        DM_AGE < 19 & ZZ_BMI_Z <= -3 & !is.na(ZZ_BMI_Z) ~ "Severe",
        .default = "Not defined"
      ),
  ...
  long_data <- long_data %>%
    mutate(
      ZZ_BMIs_GRP = case_when(
        DM_AGE >= 19 & ZZ_BMI >= 18.5 & !is.na(ZZ_BMIs) ~ "Mild_norm",
        DM_AGE >= 19 & ZZ_BMI < 18.5 & ZZ_BMI >= 16 & !is.na(ZZ_BMIs) ~ "Moderate",
        DM_AGE >= 19 & ZZ_BMI < 16 & !is.na(ZZ_BMIs) ~ "Severe",
        .default = "Not defined"
      ),
  ...
  # create dummy variables - this helps later on when calculating predictions
  long_data <- long_data %>%
    dplyr::mutate(
      INT = 1,
  ...
  # this is for debugging - create a warning if there are missing values in the imputed datasets
  if (any(sapply(long_data[long_data$.imp != 0, !names(long_data) %in% c("ZZ_BMIs_GRP", "ZZ_BMI_Z_GRP")], function(x) sum(is.na(x)))) > 0) {
    warning("NA values created")
  }

  # convert back to 'mids' class
  prog$mice$result_grp <- mice::as.mids(long_data)
  prog # return value
}
