...
PREDICTORS_INIT <- c(
  ...
  "LB_BL_ALTs", "LB_BL_CREATs", "MP_BL_SPLEEN_LENGTHs2"
)
...
data_boot <- bootstrapData(
  df = originalModel$data, bs_num = 1, cluster = CLUSTER
)
...
bootModel <- runModel(
  ...
  keep = KEEP,
  p_wald = P_WALD,
  wald_prop_thres = 50, # percentage of models that include the predictor for it to be consider in Rubin's Rules variable selection stage
  p_rr = P_RR,
